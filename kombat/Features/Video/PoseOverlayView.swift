...
/// Draws the detected skeleton over the video player. The player letterboxes
/// the video (aspect-fit), so points map through the same fit transform.
    ...
    var body: some View {
        Canvas { context, size in
            ...
            for (from, to) in DetectedPose.bones {
                ...
                path.move(to: a)
                path.addLine(to: b)
                context.stroke(path, with: .color(Theme.Colors.accent), style: StrokeStyle(lineWidth: 3, lineCap: .round))
            }
